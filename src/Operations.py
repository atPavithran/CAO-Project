...
def binaryaddition(x, y):
    st.markdown(f'<p style = "color: red; font-size: 10;">Binary addition of {x} and {y}</p>', unsafe_allow_html=True)
    result_str = f"{x} + {y}\n\n"
    x_bin = bin(int(x))[2:]
    y_bin = bin(int(y))[2:]
...
def binarysubtraction(x, y):
    result_str = f"{x} - {y}\n\n"
    st.markdown(f'<p style = "color: red; font-size: 10;">Binary subtraction of {x} from {y}</p>', unsafe_allow_html=True)
    x_bin = bin(int(x))[2:]
    y_bin = bin(int(y))[2:]
    n = max(len(x_bin), len(y_bin)) + 1
    x_bin = x_bin.zfill(n)
...
def boothmultiplication(x, y):
    ...
    result = acc + q
    result = int(result, 2)
    st.markdown(f'<p style = "color: red; font-size: 10;">Booth multiplication of {x} and {y}</p>', unsafe_allow_html=True)
    a = pd.DataFrame(steps, columns=["Count", "A", "Q", "Q0", "Operation"])
    st.table(a)
...
def restoringdivision(y, x):
    ...
    st.markdown(f'<p style = "color: red; font-size: 10;">Restoring division of {y} by {x}</p>', unsafe_allow_html=True)
    a = pd.DataFrame(steps, columns=["Count", "A", "Q", "Operation"])
    st.table(a)
